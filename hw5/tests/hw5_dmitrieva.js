...
describe('HW5_Task1_Dmitrieva ФИЛЬТРАЦИЯ ПО ЦЕНЕ', function(){
    ...
    it("Применен фильтр по ценам", () => {
        ...
        //Не придумала, что на этом шаге можно толком проверить, если попадание цен проверяем в следующем
    });
...
describe('HW5_Task2_Dmitrieva СОРТИРОВКА ПО ЦЕНЕ', function(){
    ...
    it("Применена сортировка по ценам", () => {
        ...
        phonesPage.waitForFilter();
        //Аналогично, не придумала, что на этом шаге можно толком проверить, если цены проверяем в следующем
    });
...
describe('HW5_Task3_Dmitrieva УДАЛЕНИЕ ТОВАРА ИЗ КОРЗИНЫ И ЕГО ВОССТАНОВЛЕНИЕ', function(){
